...
for i in range(len(data)):
    if "move" not in data[i]:   #hou je alleen nog de beweeg instructies over
        continue
    move = data[i].split(" ")
    del move[0]
    del move[1]
    ...
    for j in range(0,move[0]):
        try:
            place.insert(0,stapels[move[1]][0])
            del stapels[move[1]][0]
        except:
            continue
    for container in place:
        stapels[move[2]].insert(0, container)
for i in stapels:
    try:
